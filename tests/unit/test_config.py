...
class ConfigTestCase(test.TestCase):
    ...
    @mock.patch("notify.config.json")
    @mock.patch("notify.config.os.environ")
    @mock.patch("notify.config.jsonschema.validate")
    @mock.patch("notify.config.open", create=True)
    def test_get_config(self, mock_open, mock_validate, mock_environ,
        ...
        mock_environ.get.assert_called_once_with(
            "NOTIFY_CONF", "/etc/notify/config.json")
